...
compute_fft_window(spectrum_state_t *s, f32 *samples, Wave *wave)
{
    ...
    // change output file for right now
    FILE *fft_output;
    fft_output = fopen("ahhh2_output.txt", "a");
    ...
    // DEBUG: print FFT magnitudes (first few bins only)
    // for (int i = 0; i < 16; i++) {
    ...
    // could find log magnitude of freq
    // should find normalized magnitude
    // should find total energy of the frequency

    // find the normalized magnitude of each fft window
    f64 normalized_mag[FFT_WINDOW_SIZE];
    float max_mag =  0;
    for (int i = 0; i < FFT_WINDOW_SIZE; i++) {
        if (s->bin_mag[i] > max_mag) {
            max_mag = s->bin_mag[i];
        }
    }

    for (int i = 0; i < FFT_WINDOW_SIZE; i++) {
        normalized_mag[i] = s->bin_mag[i] / max_mag;
    }

    
    // find the dominant frequency of each input
    int max_ind = 0;
    float max_val = 0;
    for (int i = 0; i < FFT_WINDOW_SIZE/2; i ++) {
        if (s->bin_mag[i] > max_val) {
            max_val = s->bin_mag[i];
            max_ind = i;
        }
    }
    float freq = max_ind * s->sample_rate / FFT_WINDOW_SIZE;
    
    for (int i = 0; i < FFT_WINDOW_SIZE; i++) {
        fprintf(fft_output, "bin %d: re=%f im=%f bin_mag=%f norm_mag=%f freq=%f Hz\n", i, s->fft_out[i][0], s->fft_out[i][1], s->bin_mag[i], normalized_mag[i] ,freq);
    }
    fprintf(fft_output, "\n---------------------------------------------------------------\n");
    fclose(fft_output);
}
...
{
    fprintf(stderr, "spectrum_update called: dt=%f acc=%f sec_per_win=%f\n",
        dt, s->accumulator, s->seconds_per_window);
    fflush(stderr);
	...
	fprintf(stderr, "about to compute FFT window\n");
        fflush(stderr);
        //s->accumulator -= s->seconds_per_window;
